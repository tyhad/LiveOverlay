...
1. **[KEPUTUSAN FINAL] TikTok connector diganti ke `tiktok-live-connector`** — implementasi REST polling saat ini (`webcast.tiktok.com/webcast/room/info/`) TIDAK bisa dapat chat sama sekali (chat cuma tersedia lewat WebSocket push service TikTok). Sudah diputuskan: ganti total ke library `tiktok-live-connector` (npm, aktif maintained — 41k downloads/minggu, healthy). Listen event `WebcastEvent.CHAT` buat update `latestChatAuthor`/`latestChatMessage` real-time. **PENTING**: pastikan install package `tiktok-live-connector` (bukan fork/scope lain seperti `@adamjessop/tiktok-live-connector` yang sudah gak di-maintain setahun).
